...
//!   only the CLI token used by users to manage their resources on Cobble.
//! - **Service clients**: These requests are authenticated using *service account credentials*.
//!   This is typically used for user-created backend services that need to interact with the Cobble
...
/// interact with the Cobble API, such as creating rooms or authenticating users.
