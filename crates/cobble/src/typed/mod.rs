//! Utilities for serializing and exporting type information about Cobble apps.
//!
    ...
    pub(crate) fn export_types(&self) {
        ...
        let select_stores = self
            ...
            .map(|(_, select)| {
                ...
                cobble_schemas::typed::StoreSerialized {
                    name: desc.name,
                    select: desc.select,
                }
            })
        ...
        self.inner.platform.report_app_schema(
            &serde_json::to_string_pretty(&cobble_schemas::typed::AppSchema {
                rpcs: rpcs
                    .values()
                    .map(|rpc| {
                        let desc = (rpc.desc)(&mut generator);
                        cobble_schemas::typed::RpcSerialized {
