...
use anyhow::Context;
use cobble_schemas::{apps::RoomCreationStrategy, project_config::ProjectConfigFile};

/// Configuration information for a Cobble project, if found in the current directory or any parent
/// directory.
#[derive(Debug, Clone)]
...
impl ProjectConfig {
    pub fn load() -> anyhow::Result<Option<Self>> {
        ...
        loop {
            let config_path = current_directory.join("cobble-project.toml");
            if fs::exists(&config_path).is_ok_and(|exists| exists) {
                ...
                return Ok(Some(ProjectConfig {
                    ...
                    data: toml::from_str(&content)
                        .context("Failed to parse cobble-project.toml")?,
                }));
            }
