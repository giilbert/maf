...
This guide will help set up a simple Cobble application (both server and client)
that synchronizes a counter between multiple clients.

⚠️ **NOTE**: Cobble is still in very early development: things are not easy to do,
APIs and features are subject to change, and things will break!

## Recommended File Structure

Cobble is a full-stack framework, helping you build both server and client code.
...
to build Cobble CLI from Scratch. See [https://www.rust-lang.org/tools/install](https://www.rust-lang.org/tools/install)
...
If the installation is successful, you should be able to run the `cobble_cli`
command:
...
This will allow you to run the `cobble` command instead of `cobble_cli`. The rest of
the documentation will refer to the `cobble` command.

</TabsContent>

</Tabs>

## Write a Cobble Server
...
this by passing the `--port` flag to the `cobble run` command._
